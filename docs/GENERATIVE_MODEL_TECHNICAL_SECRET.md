...
## 5. 应用范围 (Application Scope)

本模型的核心价值在于解决光伏功率预测中的**“冷启动” (Cold Start)** 难题，特别适用于新投运场站及历史数据缺失严重的场景。

### 5.1 核心场景：新场站冷启动增强
*   **全季节分布补全 (Seasonal Augmentation)**：新设电站在投运初期通常仅有 1 个月左右的历史数据，无法捕捉全年的气象特征（如夏季强对流与冬季降雪的差异）。本模型利用提取的站点指纹，结合历史 NWP 数据，可为新站合成过去 11 个月的高保真功率序列。
*   **预测精度平滑过度**：通过将合成数据与少量真值混合训练，使下游预测模型在投运之初即可具备“全气象感知”能力，将预测达标周期从 12 个月缩短至 1 个月。

### 5.2 其他应用场景
*   **跨场站物理特性迁移**：在保护原始发电数据隐私的前提下，通过共享站点指纹原型，实现从区域内“成熟场站”向“新建场站”的知识迁移。
*   **极端天气压力测试**：利用生成模型模拟场站在极端气象（如日偏食、极端低温、重污染天气）下的出力表现，辅助电网进行调峰预演。
*   **历史数据修复**：对因采集系统故障导致的长期断数进行基于物理规律的高保真插值补全。
